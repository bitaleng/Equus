...
- **추가요금 테스트 데이터 생성기 현재 영업일 기준으로 수정**: 사용자가 바로 확인할 수 있도록 현재 영업일 데이터 생성
  - **문제**: 어제 영업일 데이터가 생성되어 오늘 홈 페이지에서 추가요금이 보이지 않음
  - **해결**: 현재 영업일 기준으로 테스트 데이터 생성 (영업일 시작 +2시간 입실 → +11시간 퇴실, 9시간 사용)
  - **보장된 시나리오**: 현재 영업일 내 9시간 사용으로 추가요금 5000원 발생 (입실/퇴실 모두 같은 영업일)
  - 3일치 과거 데이터: 각 날짜별 랜덤 10-25건 (퇴실완료, 주야간 랜덤, 락커 재사용 가능)
  - 오늘 사용중 데이터: 랜덤 15-30건 (in_use 상태, 락커 중복 방지)
  - 상세 콘솔 로그: 현재 영업일, 입실/퇴실 시간, 영업일 비교, 사용 시간, 추가요금 건수 확인
  - 검증 기능: 생성 완료 후 추가요금 건수 및 상세 내역 출력
  - 결과: 현재 홈 페이지에서 즉시 추가요금 확인 가능
