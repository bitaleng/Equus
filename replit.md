...
- **추가요금 테스트 데이터 생성 로직 완전 재작성**: 올바른 시간 시스템 규칙에 따라 같은 영업일 추가요금 생성
  ...
  - **같은 영업일 추가요금 조건**: 자정 이전 주간 입실(07:00-19:00) + 자정(00:00) 넘김 + in_use 상태
  - **보장된 시나리오**: 현재 영업일 12:00-18:00 주간 입실 → 자정(00:00) 넘김 → 추가요금 5000원 → 현재 in_use
  - 입실 영업일 = 자정 영업일 = 현재 영업일 ✓
  - 3일치 과거 데이터: 각 날짜별 랜덤 10-25건 (퇴실완료, 주야간 랜덤, 락커 재사용 가능)
  - 오늘 사용중 데이터: 랜덤 15-30건 (in_use 상태, 락커 중복 방지)
  - 상세 콘솔 로그: 현재 영업일, 입실 시간, 자정 시각, 영업일 비교, 추가요금 결제수단 확인
