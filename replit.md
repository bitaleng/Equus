...
- **추가요금 테스트 데이터 생성 로직 완전 재작성**: 올바른 시간 시스템 규칙에 따라 같은 영업일 추가요금 생성
  - **시간 시스템 규칙**: ① 주간/야간 07:00/19:00 기준, ② 추가요금 내국인 자정(00:00)/외국인 24시간, ③ 영업일 10:00~다음날 10:00
  - **핵심 원칙**: 입실요금은 입실 시간 기준 영업일, 추가요금은 **퇴실 시간 기준 영업일**에 매출 기록
  - **보장된 시나리오**: 전날 영업일 12:00-18:00 주간 입실 → 자정(00:00) 넘김 → 추가요금 5000원 발생 → 현재 in_use (그린색)
  - 입실요금: 입실 시간 기준 영업일 (전날 영업일) 매출 → 정산 완료
  - 추가요금: 퇴실 시간(현재) 기준 영업일 (오늘 영업일) 매출 ✓
  - 락커 상태: in_use → 오늘 현황에 그린색으로 표시
  - 3일치 과거 데이터: 각 날짜별 랜덤 10-25건 (퇴실완료, 주야간 랜덤, 락커 재사용 가능)
  - 오늘 사용중 데이터: 랜덤 15-30건 (in_use 상태, 락커 중복 방지)
  - 상세 콘솔 로그: 입실/추가요금 영업일, 입실 시간, 자정 시각, 추가요금 퇴실시간(현재), 결제수단 확인
  - 결과: 현재 시각과 무관하게 홈 페이지에서 즉시 추가요금 확인 가능 ✓
